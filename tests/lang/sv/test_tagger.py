...
@pytest.mark.parametrize(
    ...
    [("sv_talbanken-ud-dev01_1.json", 0.93)],
)
def test_sv_tagger_corpus(NLP, test_file, accuracy_threshold):
    data_path = TEST_FILES_DIR / test_file
    evaluate_corpus(NLP, data_path, {"tag_acc": accuracy_threshold})


def test_sv_tagger_spaces(NLP):
...
def test_sv_tagger_return_char(NLP):
    """Ensure spaces are assigned the POS tag SPACE"""
    text = (
